...
* При копировании переменной, _которая ссылается на данные в куче_, всегда создается _поверхностная_ копия объекта (из стека копируются указатель, емкость и фактически занятый переменной размер памяти). При этом происходит смена владельца, а старая переменная прекращает свое существование. Такие типы данных всегда реализует типаж `Drop`.
